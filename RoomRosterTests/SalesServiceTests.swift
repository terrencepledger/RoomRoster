...
final class SalesServiceTests: XCTestCase {
    func testRecordSaleSendsPost() async throws {
        ...
        let service = SalesService(sheetId: "sheet", networkService: mock)
    ...
    func testFetchSalesUsesNetwork() async throws {
        ...
        mock.authorizedFetchResults = [sheet]
        let service = SalesService(sheetId: "sheet", networkService: mock)
        let sales = try await service.fetchSales()
        XCTAssertEqual(mock.authorizedFetchInputs.first?.absoluteString, "https://sheets.googleapis.com/v4/spreadsheets/sheet/values/Sales")
